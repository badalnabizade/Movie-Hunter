...
credentials = service_account.Credentials.from_service_account_file("/home/badal/Downloads/sparkrecommendationengine-45d29f3b4e50.json")
    ...
    cluster_data = {
        ...
        'config': {
            'gce_cluster_config': {
                'zone_uri': zone_uri,
                "metadata": {
                    'PIP_PACKAGES': 'pandas requests beautifulsoup4'
                }
            },
            'master_config': {
                'num_instances': 1,
...
def main(project_id,
    ...
    if global_region:
        ...
        dataproc_cluster_client = dataproc_v1.ClusterControllerClient(credentials=credentials)
        dataproc_job_client = dataproc_v1.JobControllerClient(credentials=credentials)
    else:
        region = get_region_from_zone(zone)
        # Use a regional gRPC endpoint. See:
        # https://cloud.google.com/dataproc/docs/concepts/regional-endpoints
        client_transport = \
            cluster_controller_grpc_transport.ClusterControllerGrpcTransport(credentials=credentials,
                address='{}-dataproc.googleapis.com:443'.format(region))
        job_transport = \
            job_controller_grpc_transport.JobControllerGrpcTransport(credentials=credentials,
                address='{}-dataproc.googleapis.com:443'.format(region))
...
main(project_id='sparkrecommendationengine', zone='europe-west6-b', cluster_name='sparkrecommendationengine',
                                            bucket_name='sparkrecommendationengine', pyspark_file='db_test.py',
                                                                        create_new_cluster=True, global_region=False)
